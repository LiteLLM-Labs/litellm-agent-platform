...
export function renderMemoryBlock(memories: Memory[]): string {
  ...
  return [
    ...
    "",
    "SCOPE — pick priority to match how the user wants this surfaced:",
    "- ALWAYS-ON (priority 4-5): pre-loaded into the system prompt every",
    "  turn. Use for blanket style/tone/constraint rules the user wants",
    "  applied to every response.",
    "- WHEN-RELEVANT (priority 0-1): surfaced only when search_memory",
    "  matches. Use for topic-, file-, or workflow-specific guidance.",
    "If you can't tell which the user wants, ASK before saving",
    "(\"Should I apply this to every response, or only when X?\").",
    "Wrong-scope memories either crowd the prompt or never get found.",
    "",

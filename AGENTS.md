...
Neon's pgbouncer pooler is incompatible with Postgres advisory locks, which `prisma migrate deploy` requires. Use the **direct (non-pooled)** connection URL for all migration operations.

## Two connection strings

| Purpose | Env var | URL pattern |
|---|---|---|
| App queries | `DATABASE_URL` | `...pooler.us-east-1.aws.neon.tech/...` |
| Migrations | `DATABASE_URL_MIGRATE` | `...direct.us-east-1.aws.neon.tech/...` |

Both are stored in the `litellm-env` K8s secret.
...
DATABASE_URL="<direct-url>" npx prisma migrate dev --name <migration_name> --create-only
# 3. Commit prisma/migrations/XXXX_<migration_name>/migration.sql
# 4. On next deploy, web pod runs: DATABASE_URL=$DATABASE_URL_MIGRATE npx prisma migrate deploy
```

## Emergency: fix missing table without a deploy

```bash
# Uses db push (no advisory locks) against direct URL — syncs schema to DB immediately
DATABASE_URL="<direct-url>" npx prisma db push --skip-generate
```

After `db push`, mark the state as a baseline migration so future `migrate deploy` stays in sync:

```bash
# Generate baseline SQL from current schema
DATABASE_URL="<direct-url>" npx prisma migrate diff \
...
provider = "postgresql"
EOF
# Mark it applied (DB already has these tables — don't re-run)
DATABASE_URL="<direct-url>" npx prisma migrate resolve --applied 0001_init
```

## How the web pod runs migrations

Startup command: `DATABASE_URL=$DATABASE_URL_MIGRATE npx prisma migrate deploy && node server.js`

`DATABASE_URL_MIGRATE` is the direct URL from the `litellm-env` secret. This bypasses pgbouncer and lets advisory locks work.

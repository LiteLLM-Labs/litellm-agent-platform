...
// real PTY running the configured command (default: `opencode`).
//
// Protocol on /tty:
...
const CMD = process.env.POC_CMD ?? "opencode";
const REPO_DIR = process.env.REPO_DIR ?? process.cwd();

// Wrap opencode in a tmux session so it survives WS reconnects. `lap` is
...
// thereafter — so reconnecting lands on the in-progress opencode TUI with
// full conversation history instead of restarting from scratch. Killing
// the spawned pty on ws-close kills the tmux *client*, not the server,
// so opencode and its context persist for the next attach. Mirrors the
// pattern used by harnesses/claude-code and harnesses/hermes.
...
// Wire LiteLLM as the OpenAI-compatible provider for opencode at startup.
// opencode reads OPENAI_API_KEY / OPENAI_BASE_URL from the environment
// when the built-in "openai" provider is used. The entrypoint already
// writes opencode.json with a `litellm` provider entry, but setting these
// env vars too lets `opencode` work even if the config file isn't present
// (e.g. when POC_CMD=bash for debugging).
...
const server = http.createServer(async (req, res) => {
  ...
  // marking the session ready. Return a constant id so it succeeds.
